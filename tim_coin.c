...
void deanon_preprocess(blockchain_t* bc)
{
    ...
    while (bc->blocks){
        while (bc->blocks->trans){
            for (int i = 0; i < bc->blocks->trans->num_inputs; i++){
                add_address(bc->blocks->trans->inputs[i]->addr, bc->deanon);
            }
            add_associations(bc->blocks->trans->inputs, bc->blocks->trans->num_inputs, bc->deanon);
...
double cluster_balance(cluster_t* cluster, blockchain_t* bc)
{
    ...
    while(cluster){
        double abal = 0;
        if (get_balance(cluster->addr, &abal, bc->addr_bal)){
            cbal+=abal;
        }
